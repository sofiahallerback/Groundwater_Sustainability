...
dataframe <- data.frame(year = c(Data_1981_2013$V1,2014:2099), # futurePP[15:50,1]), 
                        precip = c(Data_1981_2013$V2, CAPP_2011_2099$V1[4:89,1]), #futurePP[15:50,6]), 
                        GWL_model = c(GW_level_1981_2013$x, rep(NA,86)),
                        GWL_model_senario1 = c(rep(NA,119)),
                        GWL_model_senario2 = c(rep(NA,119)),
                        past_future = c(rep("Past (Prism)",33),rep("Predicted\n(Cal-Adapt)", 86)),
...
# Unit chose outputed
  ...
  output$number1.plot <- renderPlot({
    ...
    swS = 0 #input$swS #", "Surfacewater Supply", -100, 100, c(0), step = 0.5),
    rrS = 0 #input$rrS #", "Recycle and Reuse Supply",-100, 100, c(0), step = 0.5),
    #h4("Efficenty Senario - Demand"),
    argD = 0 #input$argD #", "Agricultural Demand",-100, 100, c(0), step = 0.5),
    urbanD = 0 #input$urbanD #", "Urban Demand",-100, 100, c(0), step = 0.5),
   ...
   # }
    ...
    if (input$unit == "km^3"){
      ...
      dataframe_new$GWL_model_senario1 <- dataframe_new$GWL_model_senario1 *scale
      dataframe_new$GWL_model_senario2 <- dataframe_new$GWL_model_senario2 *scale
      dataframe_new$GWL_model <- dataframe_new$GWL_model *scale
      obsData$depletion <- obsData$depletion *scale
    ...
    ggplot() + # y=gtReactive()
      ...
      geom_line(data = dataframe_new, aes(x=year,y=GWL_model_senario2, colour = "Management Scenario"),  size=1.5)+ #color ="darkred",
      geom_line(data = dataframe_new, aes(x=year,y=GWL_model_senario1, colour = "Business as usual"),  size=1.5)+ # color = "purple",
      ...
      scale_color_manual("",
                         breaks = c("Model\nfitted with\nobservations", "Management Scenario", "Business as usual", "Observation\n(source USGS)", "Observation\n(source GRACE)"),
                          values = c( "black", "chartreuse3", "darkblue", "cornflowerblue", "darkblue"))+ #, guide = guide_legend(nrow = 1)
    ...
    # if time_end < 2013
      ...
      # Create the Goundwater level plot
      ggplot() + # y=gtReactive()
        geom_area(data = dataframe, aes(x=year,y=GWL_model*scale), fill = "white", alpha = 0.5) +
        geom_ribbon(data = dataframe, 
                    aes(x=year,ymax=GWL_model*scale,
                        ymin = min(GWL_model*scale, na.rm =TRUE)), 
                    fill = "blue", alpha = 0.3) +
        geom_line(data = dataframe, aes(x=year,y=GWL_model*scale, colour ="Model\nfitted with\nobservations"),  size=1.5)+ #, color = "darkblue" color = "black",
        geom_point(data = obsData %>% filter(Source == "Observation source: GRACE"), 
                   aes(x=as.numeric(year),y= as.numeric(depletion)*scale, colour = "Observation\n(source GRACE)"), size = 3) +
        geom_point(data = obsData %>% filter(Source == "Observation source: USGS"), 
                   aes(x=as.numeric(year),y= as.numeric(depletion)*scale, colour = "Observation\n(source USGS)"), size = 3) +
        scale_color_manual("",
                           breaks = c("Model\nfitted with\nobservations", "Observation\n(source USGS)", "Observation\n(source GRACE)"),
                           values = c( "black", "cornflowerblue", "darkblue"))+ #, guide = guide_legend(nrow = 1)
        labs(x= "Time", y = "Groundwater level", color = "")+ #title = "Groundwater level - model output and observations"
        theme_bw()+
        scale_x_continuous(breaks = c(1980,1990,2000,2010,2020,2030,2040,2050,2060,2070,2080,2090),
                           limits = c(1980, time_end)) +
 ...
 #              colour = guide_legend(order=2, override.aes=aes(fill=NA)))
    ...
    ##########################################################################################
    # 3.5 Water supply plot
    ##########################################################################################
    
    output$number4.plot <- renderPlot({
    ...
    ##########################################################################################
    # 3.6 Simulated Change in groundwater level
    ##########################################################################################
    
    output$number5.plot <- renderPlot({
      ...
      # Change the simulated time period
      sim_start = 2013
      sim_end = 2099 #max(2013, time_end)
      
      if (time_end >2013){
        # Simulation range
        dataframe_sim <- dataframe %>% filter(year > sim_start-1) #%>% filter(year < time_end)
      ...
      dataframe_new$GWL_modeled_past_1 <- c(NA, dataframe_new$GWL_model[2:length(dataframe_new$GWL_model)-1])
      
      dataframe_new$GWL_modeled_past_2 <- c(NA, dataframe_new$GWL_model_senario2[2:length(dataframe_new$GWL_model_senario2)-1])
      
    
      ggplot()+ 
        geom_bar(data = dataframe_new, 
                 aes(x=year, y= (GWL_model - GWL_modeled_past_1) * scale, fill = wetdry), stat="identity") +
        geom_bar(data = dataframe_new, 
                 aes(x=year, y= (GWL_model_senario2 - GWL_modeled_past_2) * scale, fill = wetdry), stat="identity") +
        scale_fill_manual(values = c("red", "blue")) +
        labs(x= "Time", y = "",fill="")+ #, title= "Precipitation ratio compared to average"
        theme_bw()+
        geom_hline(yintercept = 0)+
        scale_x_continuous(breaks = c(1980,1990,2000,2010,2020,2030,2040,2050,2060,2070,2080,2090),
                           limits = c(1981, time_end)) +
        theme(legend.position="right", #panel.grid.major = element_blank(), #panel.grid.minor = element_blank(),
              panel.border = element_blank(),
              # axis.text.x = element_blank(),
              #  axis.ticks.x = element_blank(),
      ...
      # If time_end < 2013
        ...
        dataframe$GWL_modeled_past_1 <- c(NA, dataframe$GWL_model[2:length(dataframe$GWL_model)-1])
    ...
    ##########################################################################################
    # 3.7 Observed Change in groundwater level
    ##########################################################################################
    
    output$number6.plot <- renderPlot({
      ...
      time_end <- input$time # [2]
        ...
        obsData$GWL_obs_past <- c(NA, obsData$depletion[2:length(obsData$depletion)-1])
        obsData$wetdry <- dataframe$wetdry[1:length(obsData$depletion)]
        
        ggplot()+ 
          geom_bar(data = obsData, 
                   aes(x=year, y= (depletion - GWL_obs_past) * scale, fill = wetdry), stat="identity") +
          scale_fill_manual(values = c("red", "blue")) +
          labs(x= "Time", y = "", fill = "")+ #, title= "Precipitation ratio compared to average"
          theme_bw()+
          geom_hline(yintercept = 0)+
